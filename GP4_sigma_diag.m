...
num_test = 20;
results = zeros(1,num_test);
norms = zeros(1,num_test);
...
for test = 1:num_test
    ...
    norms(test) = norm(sigma-sigma_diag,'fro');
    ...
    for iter = 1:iterations
        ...
        results(test) = results(test)+total_cost;
    end

    results(test) = results(test)/iterations;
    disp(['Average over ',num2str(iterations),' iterations is ',num2str(results(test))]);
end

scatter(norms,results);
